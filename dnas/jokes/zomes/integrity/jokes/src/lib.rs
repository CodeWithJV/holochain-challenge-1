    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::Joke(joke) => {
                    validate_create_joke(EntryCreationAction::Create(action), joke)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::Joke(joke) => {
                    validate_create_joke(EntryCreationAction::Update(action), joke)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::Joke(joke) => {
                        ...
                        let original_joke = match Joke::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Joke from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_joke(action, joke, original_create_action, original_joke)
                    }
                }
            }
            _ => Ok(ValidateCallbackResult::Valid),
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::Joke(original_joke) => validate_delete_joke(
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::CreatorToJokes => {
                validate_create_link_creator_to_jokes(action, base_address, target_address, tag)
            }
            LinkTypes::JokeUpdates => {
                validate_create_link_joke_updates(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::CreatorToJokes => validate_delete_link_creator_to_jokes(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::JokeUpdates => validate_delete_link_joke_updates(
                action,
                original_action,
                base_address,
                target_address,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                    ...
                    match app_entry {
                        EntryTypes::Joke(joke) => {
                            ...
                            if let ValidateCallbackResult::Valid = result {
                                ...
                                validate_update_joke(action, joke, original_action, original_joke)
                            } else {
                                Ok(result)
                            }
                        }
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::CreatorToJokes => validate_create_link_creator_to_jokes(
                        action,
                        base_address,
                        target_address,
                        tag,
